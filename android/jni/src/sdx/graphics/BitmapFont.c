...
void sdx_graphics_bitmap_font_data_load (sdxgraphicsBitmapFontData* self, sdxfilesFileHandle* fontFile, gboolean flip) {
	...
	self->lineHeight = (gfloat) ((gint) _tmp41_);
		...
		for (ch_it = ch_collection; ch_it != NULL; ch_it = ch_it->next) {
			...
			ch = _tmp102_;
			{
				...
				if (_tmp161_) {
					...
					_tmp170_ = fminf ((gfloat) (_tmp166_ + _tmp168_), _tmp169_);
	...
	self->spaceWidth = (gfloat) _tmp194_;
	xGlyph = NULL;
	...
	if (_tmp208_ == NULL) {
					...
					}
					{
						...
						g = 0;
						{
							...
							while (TRUE) {
								...
								_tmp230_ = fmaxf (_tmp227_, (gfloat) _tmp229_);
		...
		self->capHeight = (gfloat) _tmp232_;
	}
