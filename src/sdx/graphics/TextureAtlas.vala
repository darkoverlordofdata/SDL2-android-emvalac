...
namespace sdx.graphics {
    ...
    public class TextureAtlas : Object {
        ...
        public Sprite? createSprite(string name, int index=-1) { 
		    //  Android.logWrite(Android.LogPriority.ERROR, "createSprite", name);
		    //  Android.logWrite(Android.LogPriority.ERROR, "createSprite", "Len = "+regions.length().to_string());
            foreach (var region in regions) {
                if (index == -1) {
		            //  Android.logWrite(Android.LogPriority.ERROR, "createSprite1", region.name);
                    if (region.name == name) {
		                //  Android.logWrite(Android.LogPriority.ERROR, "createSprite2", region.name);
        ...
        public void load(TextureAtlasData data) {
            ...
            foreach (var page in data.pages) {
		        ...
		        //  Android.logWrite(Android.LogPriority.ERROR, "Page", page.textureFile.getPath());
            ...
            foreach (var region in data.regions) {
		        //  Android.logWrite(Android.LogPriority.ERROR, "Region", region.name);
                var width = region.width;
                var height = region.height;
                var atlasRegion = new AtlasRegion(pageToTexture.@get(region.page.id.to_string()), region.left, region.top,
				    region.rotate ? height : width, region.rotate ? width : height);

		        //  Android.logWrite(Android.LogPriority.ERROR, "Region2", region.name);
